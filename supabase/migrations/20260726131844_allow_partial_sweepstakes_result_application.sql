-- A report can contain a mix of approved and rejected rows.  Applying the
-- report must add only approved rows that have not already been recorded.
-- This keeps the operation safe to retry and avoids rejecting a whole report
-- because one result was intentionally excluded.
...
declare
  ...
  v_previously_applied_rows integer := 0;
...
begin
  ...
  if exists (
    select 1 from public.club_sweepstakes_result_import_rows
    where import_id = p_import_id and status not in ('approved', 'rejected')
  ) then
    raise exception 'Approve or reject every result row before applying this report.';
  end if;

  select count(*) into v_previously_applied_rows
  from public.club_sweepstakes_result_import_rows result_row
  join public.club_sweepstakes_result_applications application
    on application.result_row_id = result_row.id
  where result_row.import_id = p_import_id
    and result_row.status = 'approved';
  ...
  for v_row in
    ...
    where result_row.import_id = p_import_id
      and result_row.status = 'approved'
      and not exists (
        select 1 from public.club_sweepstakes_result_applications application
        where application.result_row_id = result_row.id
      )
    order by result_row.created_at, result_row.id
  loop
  ...
  return jsonb_build_object(
    ...
    'previously_applied_rows', v_previously_applied_rows,
