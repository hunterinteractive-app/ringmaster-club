-- Provision the sanction options submitted during onboarding after the club
-- record exists. Keeping this in a trigger avoids duplicating the larger
-- approval function and makes the insert idempotent if an approved draft is
-- subsequently updated by an administrator.
...
revoke all on function public.provision_onboarding_sanction_types() from public;
revoke all on function public.provision_onboarding_sanction_types() from anon, authenticated;

drop trigger if exists provision_onboarding_sanction_types on public.club_onboarding_drafts;
create trigger provision_onboarding_sanction_types
after update of status, provisioned_club_id on public.club_onboarding_drafts
for each row
when (new.status = 'approved' and new.provisioned_club_id is not null)
execute function public.provision_onboarding_sanction_types();
